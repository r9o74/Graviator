            ...
            for (const entity of this.entities) {
                ...
                if (distSq < Math.pow(entity.radius + ITEM_AREA_RADIUS, 2)) {
                    if (item.type === ItemType.MASS_BOOST) {
                        entity.startMassBoost(POWERUP_DURATION);
                        ...
                        for (let s = 0; s < SATELLITE_NUM; s++) {
                            ...
                            sat.vel = new Vector2(Math.cos(launchAngle), Math.sin(launchAngle)).scale(100);
                            this.entities.push(sat);
                        }
                    } else if (item.type === ItemType.INVISIBILITY) { entity.stealthTimer = STEALTH_TOTAL_DURATION; }
                    else if (item.type === ItemType.GRAVITY_WAVE) { entity.waveChargeCount = WAVE_MAX_CHARGE; entity.waveChargeTimer = WAVE_WAITING; }
                            ...
                            switch (targetToSteal.type) {
                                case 'MASS':
                                    if (victim.powerupTimer > 0) {
                                        capturer.startMassBoost(Math.min(POWERUP_DURATION, capturer.powerupTimer + CAPTURE_EXTENTION_TIME));
                                        victim.loseMassBoostToCapture();
                                ...
                                case 'STEALTH':
                                    if (victim.stealthTimer > 0) {
                                        capturer.stealthTimer = Math.min(STEALTH_TOTAL_DURATION, capturer.stealthTimer + CAPTURE_EXTENTION_TIME);
                                ...
                                case 'INVERSION':
                                    if (victim.inversionTimer > 0) {
                                        capturer.inversionTimer = Math.min(INVERSION_DURATION, capturer.inversionTimer + CAPTURE_EXTENTION_TIME);
                                ...
                                case 'TRAIL':
                                    if (victim.repulsiveTrailTimer > 0) {
                                        capturer.repulsiveTrailTimer = Math.min(REPULSIVE_TRAIL_DURATION, capturer.repulsiveTrailTimer + CAPTURE_EXTENTION_TIME);
                                        capturer.trail = [];
                                ...
                                case 'WAVE':
                                    if (victim.waveChargeCount > 0) {
                                        // 回数制限タイプは回数を最大値にリセット
                                        capturer.waveChargeCount = WAVE_MAX_CHARGE;
                                ...
                                case 'RAMJET':
                                    if (victim.ramjetTimer > 0) {
                                        capturer.ramjetTimer = Math.min(RAMJET_DURATION, capturer.ramjetTimer + CAPTURE_EXTENTION_TIME);
        ...
        for (let i = 0; i < this.entities.length; i++) {
            for (let j = i + 1; j < this.entities.length; j++) {
                ...
                if (distSq > 0) {
                    ...
                    // 「相手に与える力」の倍率（加算プール・拡張ポイント）
                    // --- Aが与える力（fOnBに適用） ---
                    let outSignA = 1; let outMagSumA = 0; let outActiveA = false;
                    if (aInv) { outMagSumA += (INVERSION_MULTIPLE_1 - 1); outActiveA = true; }
                    let cosA = 1;
                    if (A.isRamjetActive()) {
                        const V_A = A.vel.normalize();
                        const D_B = B.pos.subtract(A.pos).normalize();
                        cosA = V_A.dot(D_B);
                        const gain_A = cosA > 0 ? RAMJET_FRONT_GAIN : RAMJET_REAR_GAIN;
                        outSignA *= Math.sign(gain_A);
                        outMagSumA += (Math.abs(gain_A) - 1);
                        outActiveA = true;
                    ...
                    if (bInv) { outMagSumB += (INVERSION_MULTIPLE_1 - 1); outActiveB = true; }
                    let cosB = 1;
                    if (B.isRamjetActive()) {
                        const V_B = B.vel.normalize();
                        const D_A = A.pos.subtract(B.pos).normalize();
                        cosB = V_B.dot(D_A);
                        const gain_B = cosB > 0 ? RAMJET_FRONT_GAIN : RAMJET_REAR_GAIN;
                        outSignB *= Math.sign(gain_B);
                        outMagSumB += (Math.abs(gain_B) - 1);
                        outActiveB = true;
                    ...
                    if (bInv) fOnB = fOnB.scale(INVERSION_MULTIPLE_2);
                    if (A.isStealthActive()) fOnA = fOnA.scale(GRAVITY_REDUCTION);
                    if (B.isStealthActive()) fOnB = fOnB.scale(GRAVITY_REDUCTION);

                    // 質量増加終了後のクールダウン：アイテム効果を保持していない相手との重力を相互に軽減
                    if (A.massBoostCoolingTimer > 0 && !B.hasActiveItemEffect()) {
                        const coolFactor = A.getMassBoostCoolingFactor();
                        fOnA = fOnA.scale(coolFactor); fOnB = fOnB.scale(coolFactor);
                    }
                    if (B.massBoostCoolingTimer > 0 && !A.hasActiveItemEffect()) {
                        const coolFactor = B.getMassBoostCoolingFactor();
                        fOnA = fOnA.scale(coolFactor); fOnB = fOnB.scale(coolFactor);
                    }
